...
- Article field extraction: title, publication date, author, content, URL, and metadata.
...
7. Links are restricted to the official source host or its subdomains, then duplicate URLs are removed in-memory before article downloads.
8. Every normalized article URL is downloaded by the article extraction Code node.
9. The workflow extracts title, publication date, author, content, URL, and metadata from common HTML patterns.
